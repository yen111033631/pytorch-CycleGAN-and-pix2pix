...
class QtOpt(nn.Module):
    def __init__(self, in_channels=1, num_actions=5):
        super(QtOpt, self).__init__()
...
class DQN(nn.Module):
    def __init__(self, 
        ...
        with torch.no_grad():
            state_tensor = torch.as_tensor(obvervation_space.sample()[None]).float().permute(0, 3, 1, 2)
            n_flatten = self.cnn(state_tensor).shape[1]
        ...
        if isinstance(self.action_space, spaces.Discrete):
            ...
            self.one_dimension = nn.Linear(512, num_actions)
        elif isinstance(self.action_space, spaces.MultiDiscrete):
            self.action_space.nvec
            self.action_x = nn.Linear(512, self.action_space.nvec[0])
            self.action_y = nn.Linear(512, self.action_space.nvec[1])
            self.action_z = nn.Linear(512, self.action_space.nvec[2])
...
class DQNAgent:
    def __init__(self, 
        ...
        obvervation_space = env.observation_space
        ...
        # try 
        self.DQN = DQN(in_channels = state_channel, action_space = self.action_space, obvervation_space=obvervation_space)
        self.DQN_target = DQN(in_channels = state_channel, action_space = self.action_space, obvervation_space=obvervation_space)
        
        # self.DQN = DQN(in_channels = state_channel, num_actions = self.action_space.n, obvervation_space=obvervation_space)
        # self.DQN_target = DQN(in_channels = state_channel, num_actions = self.action_space.n, obvervation_space=obvervation_space)
        # -----------------------------
